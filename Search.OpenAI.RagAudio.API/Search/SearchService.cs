...
internal sealed partial class SearchService
{
    ...
    {
        var searchTool = _toolFactory.Create(
            "search",
            Schemas.SearchTookSchema,
            ToolResultDestination.Server,
            SearchAsync);

        _toolRegistry.RegisterTool(searchTool);

        var groundingTool = _toolFactory.Create(
            "report_grounding",
            Schemas.GroundingToolSchema,
            ToolResultDestination.Client,
            ReportGroundingAsync);

        _toolRegistry.RegisterTool(groundingTool);
    }

    private async Task<string?> SearchAsync(string? args)
    {
        if (args is null)
        {
            return null;
    ...
    private async Task<string?> ReportGroundingAsync(string? args)
    {
        if (args is null)
        {
            return null;
        ...
        var sources = JsonSerializer.Deserialize(
                args, SerializationContext.Default.GroundingArgs)!
            .Sources
